// The Pins declaration

// Traffic Post 1
// GREENONE is LED_BUILTIN
int YELLOWONE = 12;
int REDONE = 11;

// Traffic Post 2
int GREENTWO = 10;
int YELLOWTWO = 1;
int REDTWO = 0;

// Traffic Post 3
// Analog pins A3 - A5

// Traffic Post 4
//Analog Pins A0 - A2


// Countdown pins
int A = 7;   //Defines all pins on the Arduino Uno board in order of connection.
int B = 6;
int C = 4;  // DOT is pin 5, not used in this example.
int D = 3;
int E = 2;
int F = 8;
int G = 9;

byte num0 = 0x3F;  //Hexadecimal format based upon the A-G, 0-9 Chart in excel and the wiring      // of the segment (refer to the on/off table image below).
byte num1 = 0x6;
byte num2 = 0x5B;
byte num3 = 0x4F;
byte num4 = 0x66;
byte num5 = 0x6D;
byte num6 = 0x7C;
byte num7 = 0x7;
byte num8 = 0x7F;
byte num9 = 0x6F;

void on(byte num)   // This function turns on the correct pins to display numbers passed to it         // through the variable "num".
{
  int result = bitRead(num, 0);  // Read the first binary entry in num and stores it in result.
 
      if (result == 1)  // Check to see if this segment should be on.
 
    {digitalWrite(A, HIGH);}   // Turns on the segment.
    else   // Otherwise, it turns it off.
    {digitalWrite(A, LOW);}  // Turns segment off.
 
      result = bitRead( num, 1);  // Same thing for the 6 remaining segments.
 
      if (result == 1)
 
    {digitalWrite(B, HIGH);}
    else
    {digitalWrite(B, LOW);}     
    result = bitRead( num, 2);
 
      if (result == 1)
 
    {digitalWrite(C, HIGH);}
    else
    {digitalWrite(C, LOW);}    
   result = bitRead( num, 3);
 
      if (result == 1)
 
    {digitalWrite(D, HIGH);}
    else
    {digitalWrite(D, LOW);}    
   result = bitRead( num, 4);
 
      if (result == 1)
    {digitalWrite(E, HIGH);}
    else
    {digitalWrite(E, LOW);}
    
   result = bitRead( num, 5);  
      if (result == 1)
    {digitalWrite(F, HIGH);}
    else
    {digitalWrite(F, LOW);}    
    
   result = bitRead( num, 6);
      if (result == 1)
    {digitalWrite(G, HIGH);}
    else
    {digitalWrite(G, LOW);}
  }
void setup() {
  // initialize digital pin LED_BUILTIN as an output.
  pinMode(LED_BUILTIN, OUTPUT);
  pinMode(YELLOWONE, OUTPUT);
  pinMode(REDONE, OUTPUT);

  pinMode(GREENTWO, OUTPUT);
  pinMode(YELLOWTWO, OUTPUT);
  pinMode(REDTWO, OUTPUT);

  pinMode(A3, OUTPUT);
  pinMode(A4, OUTPUT);
  pinMode(A5, OUTPUT);

  pinMode(A0, OUTPUT);
  pinMode(A1, OUTPUT);
  pinMode(A2, OUTPUT);

  // Countdown
  pinMode(A, OUTPUT); // Making all pins outputs
  pinMode(B, OUTPUT);
  pinMode(C, OUTPUT);
  pinMode(D, OUTPUT);
  pinMode(E, OUTPUT);
  pinMode(F, OUTPUT);
  pinMode(G, OUTPUT);
  pinMode(10,OUTPUT);
}

// the loop function runs over and over again forever
void loop() {
  // TRAFFIC LIGHT ONE
  digitalWrite(LED_BUILTIN, HIGH);  // turn the LED on (HIGH is the voltage level)
  digitalWrite(REDONE, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWONE, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light two must stay off
  digitalWrite(GREENTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDTWO, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light three must stay off
  digitalWrite(A3, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A4, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A5, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light 4 must stay off
  digitalWrite(A0, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A1, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A2, LOW);   // turn the LED off by making the voltage LOW

  on(num0);   // Passing "num0" to the function  "on()" defined above to display "0"
 delay(1000);   // Delay for 1 second to see the "0"
  on(num1);  // Change to "1"
  delay(1000); 
  on(num2);  
  delay(1000);
  on(num3);  
  delay(1000);
  on(num4);  
  delay(1000);
  on(num5);  
  delay(1000);
  on(num6);  
  delay(1000);
  on(num7);
  delay(1000);
  on(num8);  
  delay(1000);  
  on(num9);  
  delay(1000);
  // wait for ten second

  on(num0);   // Passing "num0" to the function  "on()" defined above to display "0"
  digitalWrite(REDONE, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(LED_BUILTIN, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWONE, HIGH);  // turn the LED on (HIGH is the voltage level)
  // Traffic Light two must stay off
  digitalWrite(GREENTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDTWO, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light three must stay off
  digitalWrite(A3, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A4, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A5, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light 4 must stay off
  digitalWrite(A0, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A1, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A2, LOW);   // turn the LED off by making the voltage LOW
  delay(2000);                      // wait for two second

  digitalWrite(REDONE, HIGH);  // turn the LED on (HIGH is the voltage level)
  digitalWrite(YELLOWONE, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(LED_BUILTIN, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light two must stay off
  digitalWrite(GREENTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDTWO, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light three must stay off
  digitalWrite(A3, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A4, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A5, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light 4 must stay off
  digitalWrite(A0, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A1, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A2, LOW);   // turn the LED off by making the voltage LOW
  on(num0);   // Passing "num0" to the function  "on()" defined above to display "0"
 delay(1000);   // Delay for 1 second to see the "0"
  on(num1);  // Change to "1"
  delay(1000); 
  on(num2);  
  delay(1000);
  on(num3);  
  delay(1000);
  on(num4);  
  delay(1000);
  on(num5);  
  delay(1000);
  on(num6);  
  delay(1000);
  on(num7);
  delay(1000);
  on(num8);  
  delay(1000);  
  on(num9);  
  delay(1000);
  // wait for ten second

  // TRAFFIC LIGHT TWO
  digitalWrite(GREENTWO, HIGH);  // turn the LED on (HIGH is the voltage level)
  digitalWrite(YELLOWTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDTWO, LOW);   // turn the LED off by making the voltage LOW
  // Traffic LIGHT ONe must stay off
  digitalWrite(LED_BUILTIN, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWONE, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDONE, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light three must stay off
  digitalWrite(A3, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A4, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A5, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light 4 must stay off
  digitalWrite(A0, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A1, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A2, LOW);   // turn the LED off by making the voltage LOW
  on(num0);   // Passing "num0" to the function  "on()" defined above to display "0"
 delay(1000);   // Delay for 1 second to see the "0"
  on(num1);  // Change to "1"
  delay(1000); 
  on(num2);  
  delay(1000);
  on(num3);  
  delay(1000);
  on(num4);  
  delay(1000);
  on(num5);  
  delay(1000);
  on(num6);  
  delay(1000);
  on(num7);
  delay(1000);
  on(num8);  
  delay(1000);  
  on(num9);  
  delay(1000);
  // wait for ten second

  
  on(num0);   // Passing "num0" to the function  "on()" defined above to display "0"
  digitalWrite(GREENTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWTWO, HIGH);  // turn the LED on (HIGH is the voltage level)
  digitalWrite(REDTWO, LOW);   // turn the LED off by making the voltage LOW
  // Traffic LIGHT ONe must stay off
  digitalWrite(LED_BUILTIN, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWONE, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDONE, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light three must stay off
  digitalWrite(A3, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A4, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A5, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light 4 must stay off
  digitalWrite(A0, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A1, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A2, LOW);   // turn the LED off by making the voltage LOW
  delay(2000);                      // wait for two second

  digitalWrite(REDTWO, HIGH);  // turn the LED on (HIGH is the voltage level)
  digitalWrite(YELLOWTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(GREENTWO, LOW);   // turn the LED off by making the voltage LOW
  // Traffic LIGHT ONe must stay off
  digitalWrite(LED_BUILTIN, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWONE, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDONE, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light three must stay off
  digitalWrite(A3, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A4, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A5, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light 4 must stay off
  digitalWrite(A0, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A1, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A2, LOW);   // turn the LED off by making the voltage LOW
  on(num0);   // Passing "num0" to the function  "on()" defined above to display "0"
 delay(1000);   // Delay for 1 second to see the "0"
    on(num1);  // Change to "1"
  delay(1000); 
  on(num2);  
  delay(1000);
  on(num3);  
  delay(1000);
  on(num4);  
  delay(1000);
  on(num5);  
  delay(1000);
  on(num6);  
  delay(1000);
  on(num7);
  delay(1000);
  on(num8);  
  delay(1000);  
  on(num9);  
  delay(1000);
  // wait for ten second


  // TRAFFIC LIGHT THREE
  digitalWrite(A3, HIGH);   // turn the LED off by making the voltage LOW
  digitalWrite(A4, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A5, LOW);   // turn the LED off by making the voltage LOW
  // Traffic LIGHT ONe must stay off
  digitalWrite(LED_BUILTIN, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWONE, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDONE, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light two must stay off
  digitalWrite(GREENTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDTWO, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light 4 must stay off
  digitalWrite(A0, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A1, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A2, LOW);   // turn the LED off by making the voltage LOW
  on(num0);   // Passing "num0" to the function  "on()" defined above to display "0"
 delay(1000);   // Delay for 1 second to see the "0"
    on(num1);  // Change to "1"
  delay(1000); 
  on(num2);  
  delay(1000);
  on(num3);  
  delay(1000);
  on(num4);  
  delay(1000);
  on(num5);  
  delay(1000);
  on(num6);  
  delay(1000);
  on(num7);
  delay(1000);
  on(num8);  
  delay(1000);  
  on(num9);  
  delay(1000);
  // wait for ten second
  
  on(num0);   // Passing "num0" to the function  "on()" defined above to display "0"
  digitalWrite(A3, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A4, HIGH);   // turn the LED off by making the voltage LOW
  digitalWrite(A5, LOW);   // turn the LED off by making the voltage LOW
  // Traffic LIGHT ONe must stay off
  digitalWrite(LED_BUILTIN, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWONE, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDONE, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light two must stay off
  digitalWrite(GREENTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDTWO, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light 4 must stay off
  digitalWrite(A0, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A1, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A2, LOW);   // turn the LED off by making the voltage LOW
  delay(2000);                      // wait for ten second

  digitalWrite(A3, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A4, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A5, HIGH);   // turn the LED off by making the voltage LOW
  // Traffic LIGHT ONe must stay off
  digitalWrite(LED_BUILTIN, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWONE, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDONE, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light two must stay off
  digitalWrite(GREENTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDTWO, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light 4 must stay off
  digitalWrite(A0, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A1, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A2, LOW);   // turn the LED off by making the voltage LOW
  on(num0);   // Passing "num0" to the function  "on()" defined above to display "0"
 delay(1000);   // Delay for 1 second to see the "0"
    on(num1);  // Change to "1"
  delay(1000); 
  on(num2);  
  delay(1000);
  on(num3);  
  delay(1000);
  on(num4);  
  delay(1000);
  on(num5);  
  delay(1000);
  on(num6);  
  delay(1000);
  on(num7);
  delay(1000);
  on(num8);  
  delay(1000);  
  on(num9);  
  delay(1000);
  // wait for ten second
  


  // TRAFFIC LIGHT 4
  digitalWrite(A0, HIGH);   // turn the LED off by making the voltage LOW
  digitalWrite(A1, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A2, LOW);   // turn the LED off by making the voltage LOW
  // Traffic LIGHT ONe must stay off
  digitalWrite(LED_BUILTIN, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWONE, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDONE, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light two must stay off
  digitalWrite(GREENTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDTWO, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light three must stay off
  digitalWrite(A3, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A4, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A5, LOW);   // turn the LED off by making the voltage LOW
  on(num0);   // Passing "num0" to the function  "on()" defined above to display "0"
 delay(1000);   // Delay for 1 second to see the "0"
    on(num1);  // Change to "1"
  delay(1000); 
  on(num2);  
  delay(1000);
  on(num3);  
  delay(1000);
  on(num4);  
  delay(1000);
  on(num5);  
  delay(1000);
  on(num6);  
  delay(1000);
  on(num7);
  delay(1000);
  on(num8);  
  delay(1000);  
  on(num9);  
  delay(1000);
  // wait for ten second

  
  on(num0);   // Passing "num0" to the function  "on()" defined above to display "0"
  digitalWrite(A0, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A1, HIGH);   // turn the LED off by making the voltage LOW
  digitalWrite(A2, LOW);   // turn the LED off by making the voltage LOW
  // Traffic LIGHT ONe must stay off
  digitalWrite(LED_BUILTIN, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWONE, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDONE, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light two must stay off
  digitalWrite(GREENTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDTWO, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light three must stay off
  digitalWrite(A3, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A4, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A5, LOW);   // turn the LED off by making the voltage LOW
  delay(2000);                      // wait for ten second

  digitalWrite(A0, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A1, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A2, HIGH);   // turn the LED off by making the voltage LOW
  // Traffic LIGHT ONe must stay off
  digitalWrite(LED_BUILTIN, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWONE, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDONE, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light two must stay off
  digitalWrite(GREENTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(YELLOWTWO, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(REDTWO, LOW);   // turn the LED off by making the voltage LOW
  // Traffic Light three must stay off
  digitalWrite(A3, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A4, LOW);   // turn the LED off by making the voltage LOW
  digitalWrite(A5, LOW);   // turn the LED off by making the voltage LOW
  on(num0);   // Passing "num0" to the function  "on()" defined above to display "0"
 delay(1000);   // Delay for 1 second to see the "0"
    on(num1);  // Change to "1"
  delay(1000); 
  on(num2);  
  delay(1000);
  on(num3);  
  delay(1000);
  on(num4);  
  delay(1000);
  on(num5);  
  delay(1000);
  on(num6);  
  delay(1000);
  on(num7);
  delay(1000);
  on(num8);  
  delay(1000);  
  on(num9);  
  delay(1000);
  // wait for ten second 
}
